        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input type="text" placeholder="Search transcripts..." className="w-full pl-9 pr-4 py-2 bg-muted border-none rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary text-foreground" />
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-border text-foreground">
                {[
                  { time: 'Oct 24, 14:32:11', query: 'What are the side effects of Lisinopril?', topic: 'Medication', topicColor: 'bg-secondary text-secondary-foreground', tokens: 145 },
                  { time: 'Oct 24, 14:30:05', query: 'How to manage type 2 diabetes diet?', topic: 'Chronic Disease', topicColor: 'bg-secondary text-secondary-foreground', tokens: 312 },
                  { time: 'Oct 24, 14:28:40', query: 'I have a headache and fever', topic: 'Symptoms', topicColor: 'bg-secondary text-secondary-foreground', tokens: 89 },
                  { time: 'Oct 24, 14:25:12', query: 'Can I take tylenol while pregnant?', topic: 'Pregnancy/Med', topicColor: 'bg-amber-500/10 text-amber-600 border border-amber-500/20', tokens: 204 },
                ].map((log) => (
                  <tr key={log.time} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4 text-muted-foreground">{log.time}</td>
                    <td className="px-6 py-4 font-medium truncate max-w-[200px]">{log.query}</td>
                    <td className="px-6 py-4"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${log.topicColor}`}>{log.topic}</span></td>
                    <td className="px-6 py-4 text-muted-foreground">{log.tokens}</td>
                  </tr>
                ))}
              </tbody>
            </table>
